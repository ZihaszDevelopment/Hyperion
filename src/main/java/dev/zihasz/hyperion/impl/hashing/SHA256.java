...
public class SHA256 implements IHash {
    ...
    public String hash(String input) {
        ...
        int[] words = HashUtils.pad(message);
        ...
        for (int i = 0, n = words.length / 16; i < n; ++i) {
            ...
            for (int t = 16; t < W.length; ++t) {
                W[t] = Signing.smallSig1(W[t - 2]) + W[t - 7] + Signing.smallSig0(W[t - 15]) + W[t - 16];
            }
            ...
            for (int t = 0; t < W.length; ++t) {
                int t1 = TEMP[7] + Signing.bigSig1(TEMP[4]) + HashUtils.ch(TEMP[4], TEMP[5], TEMP[6]) + K[t] + W[t];
                int t2 = Signing.bigSig0(TEMP[0]) + HashUtils.maj(TEMP[0], TEMP[1], TEMP[2]);
        ...
        return new String(ByteUtils.toByteArray(H));
    }
}
